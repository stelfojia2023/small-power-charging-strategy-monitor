...
- `../cache/evidence_index.md`
- `../cache/freshness_rules.md`
- `../cache/unresolved_internal_gaps.md`
- relevant files under `../cache/market_policy/`, `../cache/competitors/`, `../cache/customers/`, and `../cache/product_strategy/` before searching externally
- relevant rules under `../logic/` before making strategic decisions
...
The prompt must show a numbered menu, wait for the user's selection, then execute the selected workflow and generate at least one Markdown report under `reports/`.
...
The launcher creates the correct report skeleton in `reports/` and embeds the Codex execution prompt at the top of the file.
...
Use the local evidence cache first, then refresh only the evidence that is missing, stale, expired, or material to the current decision. Follow `inputs/source_policy.md`, `../sources/source_registry.md`, `../sources/search_playbook.md`, `../cache/evidence_index.md`, and `../cache/freshness_rules.md`.

The local cache is not a substitute for current verification. It is a reusable evidence library. If cached evidence is still current, reuse it and cite the cache source and original URL. If cached evidence is stale or the user asks for latest/current data, refresh the source before drawing conclusions.
...
- `current`: may be reused in the report.
- `stale_check`: may be used as background, but refresh if it affects a key conclusion.
- `expired`: cannot support the current decision; refresh or downgrade to historical context.
- `internal_required`: public evidence cannot close the conclusion; list the missing internal fields.

When new external evidence is collected and is likely to be reused, update or append the relevant cache file and `../cache/evidence_index.md`.
...
2. Read the company profile, strategic assumptions, competitor watchlist, source policy, prompt ontology, internal data requirements, internal data template, and strategic thresholds from `inputs/`, plus source registry and search playbook from `../sources/`.
3. Read `../cache/evidence_index.md`, `../cache/freshness_rules.md`, `../cache/unresolved_internal_gaps.md`, and the relevant cached evidence files for the requested period, competitors, customers, products, and policies.
4. Normalize keywords and company aliases using `inputs/prompt_ontology.md`.
5. Classify cached evidence as reusable, stale, expired, or internal-required using `../cache/freshness_rules.md`.
6. Start external collection only for missing, stale, expired, or decision-critical evidence. Use the fixed sources in `../sources/source_registry.md`, then use `../sources/search_playbook.md` to answer the relevant reusable research questions.
7. Collect or refresh evidence for market demand, price, tenders, OEM procurement, installation services, competitor products, competitor customers, financing, margins, overseas moves, standards, CCC/GB/T/OCPP updates, and overseas certification.
8. When refreshed evidence is reusable, append it to the relevant cache file and update `../cache/evidence_index.md`.
9. Build or update an evidence matrix using `templates/evidence_matrix.md`.
10. Separate conclusions into:
   - Publicly verifiable.
   - Internally required.
   - Weak signal only.
   - Evidence insufficient.
11. Check internal data needs using `inputs/internal_data_requirements.md`, `inputs/internal_data_template.md`, and `../cache/unresolved_internal_gaps.md`; list missing fields explicitly and convert P0/P1 gaps into fill-in tasks.
12. Evaluate H1-H6 against `inputs/strategic_thresholds.md`. Do not rely only on qualitative wording.
13. Use `templates/strategy_scorecard.md` for monthly, quarterly, and ad hoc strategy decisions so scoring is comparable over time.
14. For each important competitor, use `templates/competitor_dossier.md`, `../logic/competitor_models/`, and the relevant `../cache/competitors/<competitor>/` files when the report needs deeper support. If the competitor is Zhida, also use `inputs/zhida_risk_model.md`.
15. Filter noise. Keep only evidence that affects demand, price, customer access, profit pool, competitor strategy, product roadmap, margin risk, R&D allocation, standards/certification, or channel strategy.
16. Validate H1-H6. Mark each assumption as `成立`, `部分变化`, `不成立`, or `证据不足`.
17. Update the competitor strategy map for Zhida, Keda Intelligent, Bull, Star Charge, Wattsaving, and Tesla China. Add other competitors only when evidence shows strategic relevance.
18. Decide the adjustment level:
   - A. 维持战略
   - B. 小幅调整
   - C. 重大调整
   - D. 立即止损
19. Convert the decision into actions for 1-4 weeks and 6-24 months. Use `templates/action_tracker.md` for action tracking.
20. For management-facing output, create or include `templates/one_page_strategy_brief.md`.
21. For engineering-facing output, create or include `templates/rd_resource_decision.md`.
22. If the trigger is low-price pressure, customer target price decline, or suspected below-cost bidding, use `templates/tco_countermeasure.md`.
23. State what should not be done. Explicitly reject attractive but unsupported moves.

For quarterly or historical synthesis runs:

1. Read `inputs/historical_report_synthesis_rules.md`.
2. Read the latest available historical artifacts under `../reports/`:
   - Recent `reports/monthly/*-small-power-charging-strategy-review.md` files, ideally the latest 3.
   - The latest `reports/evidence_matrix/*-evidence-matrix.md`.
   - The latest `reports/action_trackers/*-action-tracker.md`.
   - The latest previous `reports/quarterly/*-small-power-charging-strategy-review.md`, if present.
...
Every run must create a Markdown file under `../reports/`. Do not only answer in chat unless the user explicitly asks for chat-only output.
...
```text
reports/competitor_dossiers/<period>-<YYYY-MM-DD>-<competitor>-competitor-dossier.md
```
